                    ...
                    <Row className="show-grid">
                        <Col sm={12} md={12}>
                            {this.props.usersArray.map((user) => {
                                return (
                                    <div key={user.id} className="authorNameClass containerDetail">
                                        <Col sm={4} md={4}>
                                        <img src={user.avatarURL} width="100px" height="100px" alt="Sarah Edo" />   
                                        <div>{user.name}</div>
                                    </Col>
                                    <Col sm={4} md={4}>
                                    <div>
                                        <span style={{ fontSize: 17}}>Answered questions:  </span>
                                            <b>{Object.keys(user.answers).length}</b>
                                    </div>
                                    <div>
                                        <span style={{ fontSize: 17}}>Created questions:  </span>
